...
app.post("/api/insert/bit", async (req, res) => {
  const {
    ...
    // Backwards compatibility with your current React form
    sub1,
    sub2,
    sub3,
    sub4,
  ...
  // ------------------------------------------------------------
  // Create a promise wrapper around db.query
  // ------------------------------------------------------------
  ...
  try {
    ...
    const bitResult = await query(bitSQL, [
      airDate || null,
      title || null,
      artist || null,
      autoNum || null,
    ...
    // This is the ID generated by tblbits
    ...
    // ]
    //
    // HyperlinkID should be AUTO_INCREMENT in MySQL.
    // ==========================================================
    ...
    if (cleanHyperlinks.length > 0) {
      ...
      const hyperlinkValues = cleanHyperlinks.map(link => [
        bitID,
        link
      ]);

      await query(hyperlinkSQL, [
        hyperlinkValues
      ]);

      console.log(
        `Inserted ${cleanHyperlinks.length} hyperlink(s)`
      );
    }
    ...
    // NOT albumID.
    //
    // Therefore we use album.album here.
    // ==========================================================
    ...
    if (cleanAlbums.length > 0) {
      ...
      console.log(
        `Inserted ${cleanAlbums.length} album(s)`
      );
    }
    ...
    // You said:
    //
    // "forget celeb2_id exists and only input into celeb1_id"
    //
    // Therefore every celebrity is inserted into:
    //
    //     Celeb1_ID
    //
    // We do NOT use Celeb2_ID at all.
    //
    // The preferred frontend format is:
    //
    // celebrities: [1, 5, 8, 12]
    //
    // But we also support your CURRENT frontend:
    //
    // celebrity1
    // celebrity2
    //
    ...
    if (Array.isArray(celebrities)) {
      cleanCelebrities = celebrities.filter(Boolean);
    }

    // Backwards compatibility with your current React code
    if (cleanCelebrities.length === 0) {

      if (celebrity1) {
        cleanCelebrities.push(celebrity1);
      }

      if (celebrity2) {
        cleanCelebrities.push(celebrity2);
      }
    }
    ...
    if (cleanCelebrities.length > 0) {
      ...
      const celebrityValues = cleanCelebrities.map(celebID => [
        bitID,
        celebID
      ]);

      await query(celebritySQL, [
        celebrityValues
      ]);

      console.log(
        `Inserted ${cleanCelebrities.length} celebrity(ies)`
      );
    }
    ...
    if (sport) {
      ...
      await query(sportSQL, [
        bitID,
        sport
      ]);

      console.log("Sport inserted");
    }
    ...
    if (Array.isArray(subjects)) {
      cleanSubjects = subjects.filter(Boolean);
    }

    // Backwards compatibility with current React form
    ...
    if (cleanSubjects.length > 0) {
      ...
      await query(subjectSQL, [
        subjectValues
      ]);

      console.log(
        `Inserted ${cleanSubjects.length} subject(s)`
      );
    }
    ...
    console.log(
      `BIT ${bitID} INSERTED SUCCESSFULLY`
    );
...
app.get("/api/get/bit/edit/:bitID", (req, res) => {
  ...
  const sql = `
    SELECT
      ...
      cb.Celeb1_ID AS Celebrity1,
      cb.Celeb2_ID AS Celebrity2
    ...
    LEFT JOIN tblkeywords k
      ON k.BitID = b.BitID

    LEFT JOIN tblseason s
      ON s.BitID = b.BitID

    LEFT JOIN tblsports sp
      ON sp.BitID = b.BitID

    LEFT JOIN tblceleb cb
      ON cb.BitID = b.BitID
  ...
  db.query(sql, [bitID], (err, bitResult) => {
    if (err) {
    ...
    db.query(subjectSQL, [bitID], (err, subjectResult) => {
      if (err) {
      ...
      db.query(hyperlinkSQL, [bitID], (err, hyperlinkResult) => {
        if (err) {
          console.error("GET HYPERLINK ERROR:", err);
          return res.status(500).json({
            error: "Failed to load hyperlinks",
            details: err.message
          });
        }

        db.query(albumSQL, [bitID], (err, albumResult) => {
          if (err) {
          ...
          res.json({
            ...
            subjects: subjectResult.map(row => row.SubID),
            ...
            hyperlinks: hyperlinkResult.map(row => row.Hyperlink),

            albums: albumResult.map(row => ({
              album: row.AlbumID,
